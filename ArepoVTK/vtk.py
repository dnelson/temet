...
def getColorTableParams(fieldName):
    ...
    if fieldName == "Density":
        ct = { 'file'    : ctBase + 'ncl/WhiteBlueGreenYellowRed-dnA.cpt',
               'minmax'  : [-6.1,-2.7], # [-5.5,-2.7] for 1820_16k_nni/cellgrad # original: [-4.7,-1.6]
               'log'     : True,
               'reverse' : True,
               'gamma'   : 1.3 }
    ...
    if fieldName == "SzY":
        ct = { 'file'    : ctBase + 'oc/zeu.cpt',
               'minmax'  : [-1.0,4.5], # originally [2.5,5.6]
               'log'     : True,
               'reverse' : True,
               'gamma'   : 0.5 } 
    ...
    if fieldName == "XRay":
        ct = { 'file'    : ctBase + 'kst/03_red_temperature.cpt',
               'minmax'  : [-12.0,-3.5], # originally [-7.6,-2.5]
               'log'     : True,
               'reverse' : False,
               'gamma'   : 1.5 } 
...
def make_pyramid_all(config):
    ...
    for level in range(config['levelMax'],config['levelMin']-1,-1):
        ...
        if level != config['levelMax']:
            ...
            globalArray = scipy.ndimage.zoom( globalArray, 0.5, order=config['reduceOrder'] )
...
def make_pyramid_upper(config):
    ...
    for i in range(len(config['fileList'])):
        ...
        for j in range(numReductions):
            array = scipy.ndimage.zoom( array, 0.5, order=config['reduceOrder'] )
...
def make_pyramid_lower(config):
    ...
    for i in range(len(config['fileList'])):
        ...
        for level in range(config['levelMax'],startLevel-1,-1):
            ...
            #print("  i="+str(i)+" jRow = "+str(jRow)+" jCol="+str(jCol) + " depth="+str(levelDepth))
            ...
            for colIndex in range(nSub):
                ...
                for rowIndex in range(nSub):
                    ...
                    #print("   row col: " + str(rowIndex) + " " + str(colIndex))
                    #print("   global row col: " + str(globalRow) + " " + str(globalCol))
