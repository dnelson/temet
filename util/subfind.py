...
def assign_stellar_photometrics(i, P, StarP, mags, LogMetallicity_bins, LogAgeInGyr_bins, TableMags, atime):
    ...
    for j in range(GFM_STELLAR_PHOTOMETRICS_BANDS):
        mags[j] = (1-d_k) * (1-d_i) * TableMags[j,k1,i1] + \
...
def subfind_properties_2(candidates, Tail, Next, P, PS, SphP, StarP, BHP, atime, H_of_a, G, boxsize, 
    ...
    2.41072, 1.66573, 1.49335, 1.34853, 2.48411, 2.16556, 3.08907, 0.956437, 1.88387, 2.74096, 2.28772, 2.97042, 0.0940844, 1.52121, 
    1.00048, 1.35537, 2.36729, 2.17125, 0.67752, 0.692307, 1.8943, 0.407554, 0.657783, 0.426495, 0.0747759, 1.44043, 2.00924, 
    0.542286, 2.68916, 2.0342, 2.62093, 1.91206, 0.970703, 1.87628, 1.14982, 2.50939, 0.848391, 0.58381, 1.56003, 1.5534, 1.54524, 
    2.80665, 2.79289, 0.730284, 0.390309, 1.74462, 2.20372, 3.08843, 1.45806, 0.755776, 0.302779, 0.91858, 1.35422, 2.27953, 
    1.89358, 0.840655, 2.64957, 2.77734, 3.01598, 1.14885, 2.81684, 0.998777, 1.63618]

    StellarPhotometricsRandomAngles[:,1] = [1.68477, 1.96426, 3.13277, 4.17738, 2.57749, 5.56388, 4.91011, 5.49633, 4.68758, 5.6185, 
    2.02901, 3.01843, 1.88062, 1.40816, 0.903572, 3.71739, 1.67312, 3.6844, 0.171446, 1.39946, 0.604645, 4.24954, 2.09295, 2.25746, 
    1.30573, 5.95015, 1.85119, 5.18499, 0.387137, 3.14278, 1.15531, 2.92628, 2.90757, 1.54623, 3.7789, 5.87517, 3.47764, 2.06749, 
    2.15919, 0.590433, 0.553423, 3.71127, 2.67805, 1.01639, 5.14023, 1.178, 5.73582, 1.37643, 3.16482, 3.33026, 2.93006, 2.96468, 
    3.60805, 2.04047, 3.06509, 3.06867, 3.61902, 4.00447, 4.75386, 4.42716, 4.15663, 0.664922, 1.79173, 0.763853, 0.24633, 6.02, 
    4.96996, 5.03009, 3.81081, 0.31155, 3.20804, 5.91206, 5.40538, 6.01432, 3.05248, 4.84851, 5.54481, 1.52201, 2.06171, 1.76919, 
    0.933271, 1.4099, 6.25792, 4.30342, 0.28824, 1.12354, 5.03985, 5.94228, 1.98936, 6.0363, 1.56903, 3.63016, 3.988, 0.161651, 
    4.70989, 2.396, 1.74586, 1.83267, 0.232479, 1.31449]
    ...
    for k in range(candidates.size):
        ...
        for i in range(num):
            ...
            mags_local *= 0
            assign_stellar_photometrics(p, P, StarP, mags_local, LogMetallicity_bins, LogAgeInGyr_bins, TableMags, atime)
            ...
            for idir in range(GFM_STELLAR_PHOTOMETRICS_DIRECTIONS):
                ...
                while irad > 0:
                    ...
                    if lum_grid[idir][irad] > 0:
                        tmp_sb = factor1 - 2.5 * np.log10(lum_grid[idir][irad]) + \
...
def benchmark():
    ...
    for i in range(nLoops):
        print(i)
        count_cand, nsubs, candidates, Tail, Next, P, PS, SphP, StarP, BHP = \
          subfind(P, PS, SphP, StarP, BHP, atime, sP.units.H_of_a, sP.units.G, sP.boxSize, SofteningTable, ForceSoftening)
        candidates = subfind_properties_1(candidates)
        Subgroup = subfind_properties_2(candidates, Tail, Next, P, PS, SphP, StarP, BHP, atime, sP.units.H_of_a, sP.units.G, sP.boxSize,
                                        LogMetallicity_bins, LogAgeInGyr_bins, TableMags, SofteningTable, GrNr)
