...
def loadColors(sP, quantName):
    ...
    for i, band in enumerate(bands):
        if band in bandRenamesToFSPS: bands[i] = bandRenamesToFSPS[band]
...
def loadSimGalColors(sP, simColorsModel, colorData=None, bands=None, projs=None, rad=''):
    ...
    and return band magnitudes (if bands is a single band), convert to a color (if bands 
    ...
    if bands is not None:
        if bands[0] in ['U','V','J']: acSet = 'UVJ_'
        if len(bands) == 2 and bands[1] in ['U','V','J']: acSet = 'UVJ_'
