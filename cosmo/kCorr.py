...
def _kCorrection(r, c, redshifts, color_values):
    """ Driver, see below. """
    for i in range(redshifts.size):
        kcor = 0.0

        for j in range(c.shape[0]):
            for k in range(c.shape[1]):
                kcor += c[j][k] * np.power(redshifts[i],j) * np.power(color_values[i],k)
...
def kCorrections(filter_name, redshifts, color_name, color_values):
    ...
    assert redshifts.max() <= 0.5 # not accurate beyond

    coeffKey = filter_name + '_' + color_name.replace(' - ', '')
    assert coeffKey in coeff

    c = np.array(coeff[coeffKey])

    #kcor = 0.0 # debugging
    #for x, a in enumerate(c):
    #    for y, b in enumerate(c[x]):
    #        kcor += c[x][y] * redshifts[0]**x * color_values[0]**y

    r = np.zeros( redshifts.size, dtype=np.float32 )
