...
def combineExpandedJob(jobNum):
    ...
    for fieldName in fields:
        ...
        dset = group.create_dataset("Array", data=globalArray)
...
def makeColorBars():
    ...
    outNames = ['Gas Density [log 10$^{10}$ M$_\odot$ h$^2$ / kpc$^2$]', 
                'Gas Temperature [log K]',
                'Gas Entropy [log K/cm$^2]$',
                'Gas Metallicity [log (0.0127 * Z$_\odot$)]',
                'Gas Velocity [km/s]',
                'Sunyaev Zeldovich y-parameter [log relative units]',
                'X-Ray Luminosity [log (relative units)]',
                'Dark Matter Density [log 10$^{10}$ M$_\odot$ h$^2$ / kpc$^2$]',
                'Stellar Luminosity [log L$_\odot$ / kpc$^2$]']
    ...
    for i,fieldName in enumerate(fieldNames):
        ...
        cdict_r = list( (xx,ct['table']['r'][i],ct['table']['r'][i]) for i,xx in enumerate(ct['table']['x']))
        cdict_g = list( (xx,ct['table']['g'][i],ct['table']['g'][i]) for i,xx in enumerate(ct['table']['x']))
        cdict_b = list( (xx,ct['table']['b'][i],ct['table']['b'][i]) for i,xx in enumerate(ct['table']['x']))
