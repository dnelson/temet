...
def colorTracksSchematic(sP, bands, simColorsModel=defSimColorModel, pageNum=None):
    ...
    for dustModel in [dust_C]: #[dust_C,dust_Br]:
        ...
        if len(ww[0]) < gal_snaps.size:
            ...
            if colors_evo.ndim == 2: colors_evo = np.squeeze(colors_evo[:,ww])
            if colors_evo.ndim == 3: colors_evo = np.squeeze(colors_evo[:,:,ww])
        ...
        for i, shID in enumerate(trackIDs):
            ...
            for j, snap in enumerate(gal_snaps):
                if snap not in mpb['SnapNum']: snap -= 1 # look for adjacent match
                if snap not in mpb['SnapNum']: snap += 2 # look for adjacent match
                if snap not in mpb['SnapNum']: continue
                mpbInd = tuple(mpb['SnapNum']).index(snap)
                if snap == gal_snaps[j]: assert mpb['SubfindID'][mpbInd] == shID_evo[evoInd,j]
    ...
    for i, redshift in enumerate(cRedshifts):
        ...
        xx, yy, kde_sim = calcMstarColor2dKDE(bands, None, None, xMinMax, mag_range, 
                                              sP=sP, simColorsModel=dust_C)
    ...
    for i, shID in enumerate(trackIDs):
        ...
        for j in range(xx.size):
            if gal_redshifts[dust_C][j] not in zTicks: continue
            ax.annotate('%.1f' % gal_redshifts[dust_C][j], xy=(xx[j],yy[j]), textcoords='offset points', 
                        xytext=(-5,5), ha='right', color=l.get_color(), fontsize=8, alpha=tAlpha/2)
