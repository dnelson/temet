...
def get_cone(sP,group,config,snap_index):
    ...
    for i in range(nChunks):
        ...
        index = np.where( (x_min < pos_remapped[:,0]) & (pos_remapped[:,0] < x_max) & \
                          (y_min < pos_remapped[:,1]) & (pos_remapped[:,1] < y_max) & \
                          (z_min < pos_remapped[:,2]) & (pos_remapped[:,2] < z_max) )[0]
...
def lightcone_coordinates(sP,group,pos,vel,config,snap_index):
    ...
    z_peculiar = np.sum(r_norm*vel, axis=1) / sP.units.c_km_s
