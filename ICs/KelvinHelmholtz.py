...
def create_ics(numPartPerDim=64, filename='ics.hdf5'):
    ...
    for i in range(Nx):
        for j in range(Ny):
            ...
            vel[index,0] += w0 * np.sin(4.0*np.pi*x*np.cos(angle_rad)) * \
                           (np.exp(-(d+stripeWidth/2.0)**2.0/(2.0*sigma**2.0)) + \
                            np.exp(-(d-stripeWidth/2.0)**2.0/(2.0*sigma**2.0))) * np.sin(angle_rad)
            vel[index,1] += w0 * np.sin(4.0*np.pi*x*np.cos(angle_rad)) * \
                           (np.exp(-(d+stripeWidth/2.0)**2.0/(2.0*sigma**2.0)) + \
                            np.exp(-(d-stripeWidth/2.0)**2.0/(2.0*sigma**2.0))) * np.cos(angle_rad)
...
def create_ics2(numPartPerDim=64, filename='ics.hdf5'):
    ...
    posx2 = (mesh[0]*dx/2.).reshape(N_x*N_y*4)+0.5*dx/2.
    posy2 = (mesh[1]*dy/2.).reshape(N_x*N_y*4)+0.5*dy/2.
    ...
    mass[:N_x*N_y/4] = 1.*dx*dy
    u[:N_x*N_y/4] = P/((GAMMA-1)*1.)
    vel[:N_x*N_y/4,0] = -0.5
    ...
    if middleAtTwiceRes:
        ...
        mass[N_x*N_y/4:N_x*N_y/4+N_x*N_y*2] = 2.*dx*dy/4.
        u[N_x*N_y/4:N_x*N_y/4+N_x*N_y*2] = P/((GAMMA-1)*2.)
        ...
        mass[N_x*N_y/4:3*N_x*N_y/4] = 2.*dx*dy
        u[N_x*N_y/4:3*N_x*N_y/4] = P/((GAMMA-1)*2.)
    ...
    vel[:,1] = omega_0 * np.sin(4*np.pi*pos[:,0])* \
      (np.exp(-(pos[:,1]-0.25)**2*0.5/(sigma**2))+np.exp(-(pos[:,1]-0.75)**2*0.5/(sigma**2)))
    vel[:,2] = 0.0
